...
PREAMBLE = (
    ...
    "Layout Domain: {} layout\n"
    "Canvas Size: canvas width is {}px, canvas height is {}px\n"
    "I will give you the examples of the constraints and the layouts.\n"
    "Your output format should be the same to the example layouts.\n"
    "DO NOT generate redundant messages, just output the layouts.\n"
    "Here are the examples of the layouts."
)
...
class CompletionSerializer(Serializer):
    task_type = "layout completion. You need to generate a complete layout from a given partial layout."
...
class GenPaperSerializer(Serializer):
    task_type = (
        "paper-to-layout\n"
        "Generates a layout from a summarized text of a scientific paper. "
        "There are eight optional element types, including: Title, Author Info, Section, Text, List, Table, Figure, Caption. "
        "Please do not exceed the boundaries of the canvas. "
        "Besides, do not generate elements at the edge of the canvas, that is, reduce top: 0px and left: 0px predictions as much as possible."
    )
    constraint_type = ["Paper summary: "]
...
def build_prompt(
    ...
    return [
        {
            "role": "user",
            "content": separator_between_samples.join(prompt)
        }
    ]
