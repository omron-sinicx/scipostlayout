...
from data import RicoDataset, PubLayNetDataset, SciPostLayoutDataset, transforms
from utils import utils, os_utils
from config import add_arguments
from fid_model import LayoutNet, FIDNetV3
...
class SciPostLayoutFidDataset(SciPostLayoutDataset):
...
def main():
    ...
    model = FIDNetV3(num_label=args.num_label, max_bbox=args.max_num_elements)
    ...
    if args.dataset == 'rico':
        ...
        train_dataset = SciPostLayoutFidDataset(args, 'train')
        val_dataset = SciPostLayoutFidDataset(args, 'val')
    else:
